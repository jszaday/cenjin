...
abstract class Visitor[Context, Result] {
  ...
  def visit(node: Node)(implicit ctx: Context): Result = {
    node match {
      ...
      case templateArgument: TemplateArgument => visitTemplateArgument(templateArgument)
  ...
  def visitTemplateArgument(templateArgument: TemplateArgument)(implicit ctx: Context): Result
  ...
  def visitNamespaceAlias(namespaceAlias: NamespaceAlias)(implicit ctx: Context): Result
}
